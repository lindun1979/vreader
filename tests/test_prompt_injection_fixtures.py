...
def test_injection_text_treated_as_data_only():
    ...
    assert ex["records"][0]["solved"] is True and ex["records"][0]["rounds"] == 1  # 黄金 第1轮=2分
    assert ex["records"][0]["score"] == 2
    # 注入文本没有变成额外字段/命令：schema 只允许既定字段（v2 增 series 三字段 + solved_round）
    assert set(ex["records"][0]) == {
        "model_raw", "model_canonical", "model_series", "model_version", "model_variant",
        "bug_level", "bug_id", "solved_round", "score", "solved", "rounds", "evidence_quote", "confidence"}
