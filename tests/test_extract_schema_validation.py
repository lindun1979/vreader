...
def test_score_out_of_range_dropped():
    # 青铜最多 2 轮，solved_round=3 越界 → 丢弃 → 无有效记录 → no_content
    ex = extract.build_extract(aweme_id="v1", title="t", transcript=TX,
                               claude_text=_claude([_rec(solved_round=3)]))
    _assert_no_content(ex)


def test_gold_round_derives_score():
    tx = "黄金题 G001，模型第二轮才把它修好，勉强拿了一分。"
    r = _rec(bug_level="黄金", bug_id="G001", solved_round=2, evidence_quote="模型第二轮才把它修好，勉强拿了一分")
    ex = extract.build_extract(aweme_id="v1", title="t", transcript=tx, claude_text=_claude([r]))
    rec = ex["records"][0]
    assert rec["solved"] is True and rec["rounds"] == 2 and rec["score"] == 1  # 黄金 第2轮=1分
